<template>
    ...
    <div class="container">
        <form class="formfield w-100 py-3">
            <label for="Titre de la publication">Titre de la publication - {{currentPost}} -</label>
        ...
        <button class="btn btn-primary mx-2" @click="updatePost(currentPost)">Modifier la publication</button>
        <button class="btn btn-outline-danger mx-2" data-toggle="modal" data-target="#popup">Supprimer</button>
    </div>
...
export default {
    ...
    created(){
        let id = sessionStorage.getItem('post-info')
        this.getOnePost(id)
    }
}
</script>
